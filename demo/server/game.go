...
// TODO: Input should be of type struct{} (or any type)
type GameEvent struct {
	Input     int32 // bitfield of input commands; refer to constants above
	Timestamp int64
}

// In our implementation, the application must maintain all game objects.
// TODO: make a function in server.go so that the application can
// propagate forth updates after it is done processing any ApplyEvents triggered
// by the server's channel push.
...
type GameObject struct {
	...
	Sid         int32
	Rect        Rectangle
	Type        int32 // deprecate this
	LatestEvent GameEvent

	// "Local" state; only primary needs this
	expansion float32
	speedX    float32
	speedY    float32
}
...
func (o *GameObject) GetInterest(predTimeMs int64, pubTimeMs int64) Rectangle {
	...
	bb.X -= o.expansion
	bb.Y -= o.expansion
	bb.Width += 2 * o.expansion
	bb.Height += 2 * o.expansion

	// We expand the bounding volume predictively, based on the object's velocity;
	// described in Section 6.2 of the paper.

	if o.speedX < 0 {
		bb.X += o.speedX * float32(pubTimeMs+predTimeMs)
		bb.Width -= o.speedX * float32(pubTimeMs+predTimeMs)
	} else {
		bb.Width += o.speedX * float32(pubTimeMs+predTimeMs)
	}
	if o.speedY < 0 {
		bb.Y += o.speedY * float32(pubTimeMs+predTimeMs)
		bb.Height += o.speedY * float32(pubTimeMs+predTimeMs)
	} else {
		bb.Height += o.speedY * float32(pubTimeMs+predTimeMs)
	}
...
func (o *GameObject) GetAdjustedRect(timeDelta int64) Rectangle {
	...
	rect.X += o.speedX * float32(timeDelta)
	rect.Y += o.speedY * float32(timeDelta)
...
// TODO: turn into chan so that server takes in a chan from app on init
// and then pushes every time there is an event
// then the application can handle all this
...
func (o *GameObject) SetExpansion(expansion float32) {
...
func (o *GameObject) OldGetInterest(predTimeMs int64, pubTimeMs int64) Rectangle {
	// Expanded bounding box for interest.
	// The expansion is a configurable parameter per object type; smaller
	// means less network traffic but you can miss collisions if it is too small
	// and network conditions are bad.
...
func (o *GameObject) OldGetAdjustedRect(timeDelta int64) Rectangle {
	rect := Rectangle{
		X:      o.Rect.X,
		Y:      o.Rect.Y,
		Width:  o.Rect.Width,
